...
public class ForumController {
    ...
    public ResponseEntity<Post> getPostDetail(@PathVariable int id) {
		Post post = postRepository.findById(id).orElseThrow(()-> new ResponseStatusException(HttpStatus.NOT_FOUND, "No post with the requested ID"));
    ...
    // ) {
    //     Optional<User> user = userRepository.findByName("rucha");
    //     Optional<Post> post = postRepository.findById(id);
    ...
    public ResponseEntity<Comment> addCommentToPost(@PathVariable int postId, @RequestBody AddCommentForm addComment) {
        ...
        if (postOptional.isPresent()) {
            ...
            User user = userService.findUserByName("rucha").orElse(null);
	...
	//adding comment getmapping for retrival
	@GetMapping("/{id}/comments")
    public ResponseEntity<List<Comment>> getCommentsForPost(@PathVariable int id) {
        Optional<Post> post = postRepository.findById(id);
